...
const STALE_READY_CRIT_MINS = 20;
...
function staleTimestampClass(order: Order): string {
  ...
  if (order.status === 'ready' && !order.payment_method) {
    const readyMins = Math.floor((Date.now() - new Date(order.updated_at ?? order.created_at).getTime()) / 60_000);
    if (readyMins >= STALE_READY_CRIT_MINS) return 'text-red-600 font-semibold';
  }
    ...
    <div className="p-6 pb-24 max-w-5xl mx-auto space-y-3">
          ...
          {displayItems.map((item) =>
            item.type === 'single' ? (
              <OrderCard
                ...
                onViewDetail={staffSession?.role === 'counter' ? () => setDetailOrder(item.order) : undefined}
      ...
      {detailOrder && (
        <CompletedOrderModal
          ...
          onPrintBill={() => { handlePrintBillAction(detailOrder); }}
        />
      )}
    </div>
  );
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b">
        <div>
          ...
          <p className={cn('text-xs', staffSession?.role === 'counter' ? staleTimestampClass(order) : 'text-muted-foreground')}>
...
// CompletedOrderModal is now a shared component at @/components/dashboard/CompletedOrderModal
